"""FastAPI sub-app for the /knowledge-bridge mount.

``get_app()`` returns a FastAPI instance that mcp-skill-hub mounts under the
path declared in ``plugin.json -> web_mount.mount`` (``/knowledge-bridge``).
"""
...
def get_app() -> FastAPI:
    ...
    app = FastAPI(
        ...
        description="Cross-project knowledge graph and wiki federation",
        docs_url="/docs",
    )
