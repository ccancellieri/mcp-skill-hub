...
>
> **Model bandit removed (#130, following the #120 decision).** The "model bandit state" mentioned in the current-state table and the W2 replay design below (`record_model_reward`) refers to `router/bandit.py`, which had zero recorded rewards and was removed in the #130 zero-usage purge. `wake_session`'s actual implementation never replayed bandit state — see `src/skill_hub/server.py`.
